...
from bot.auto_depot import AutoDepot
from bot.auto_expand import AutoExpand
from bot.auto_barraks import AutoBarracks
...
class ExponentialMeatGrinder(KnowledgeBot):
    tactic_index: int
    ...
    async def create_plan(self) -> BuildOrder:
        ...
        if self.tactic_index == 0:
            ...
            self.knowledge.print("Exponential Meat Grinder engaged", "Build")
            self.attack = DodgeRampAttack(20)
            chunk = [
                Step(SupplyLeft(14), GridBuilding(UnitTypeId.SUPPLYDEPOT, 1)),
                Step(UnitReady(UnitTypeId.SUPPLYDEPOT, 1), GridBuilding(UnitTypeId.BARRACKS, 1)),
                AutoDepot(),
                AutoExpand(),
                AutoBarracks(),
        ...
        return BuildOrder(
            ...
            [Step(None, ActUnit(UnitTypeId.SCV, UnitTypeId.COMMANDCENTER, 75))],
            chunk,
...
class LadderBot(ExponentialMeatGrinder):
    @property
    def my_race(self):
        return Race.Terran
